...
- ロビー・ゲーム画面への切り替えは、`join()`をクリックした時点ではなく`onConnected`（WebRTC接続が実際に確立した後）で行う。接続失敗（`onError`/`onDisconnected`、`rejoin-rejected`含む）の際はセットアップ画面のまま留める、またはセットアップ画面に戻す。エラー文言を表示する要素は通常セットアップ画面の中にあるため、接続の成否を待たずに先に画面を切り替えてしまうと、失敗時にエラーが表示されないまま参加者が誰も表示されない空のロビーに固まって見える（2026-08、[concept](concept/)・[just-one](just-one/)でこの手順が守られておらず「4人プレイ時にマッチングが壊れる」不具合として発生）
- ゲームが正常終了（`game-end`）・中断（`game-aborted`）した時点でも`RejoinStorage.clear(gameKey)`を呼ぶ。`rejoin-rejected`受信時のクリアだけでは、最後までプレイし終えた後もsessionStorageに古いトークンが残り続け、同じタブで再度そのゲームのページを開いた際に無効な部屋への自動再接続が走り、上記と同じ「空のロビーに固まる」症状が再現してしまう
...
エントリの文言は「「〇〇」を追加」「〇〇を修正」のように、何をしたかだけを一文で簡潔に書く。ゲームのルールや遊び方の説明はここでは書かない（説明は各ツール/ゲームのカード側の役割）。修正系エントリは特に簡潔さを優先し、「〇〇のバグを修正」「〇〇のUIを修正」程度の一般的な言い方に留める（不具合の具体的な症状や変更前後の挙動は書かない）。同じ日に同じゲーム/機能に対する修正が複数あっても、個別に列挙せず1行にまとめる。対象ゲームが多い変更（多くのゲームへの機能追加、複数ゲームのバグ修正など）はゲーム名を列挙せず「様々なゲームに〇〇を追加」「様々なゲームのバグを修正」のように1行にまとめる（対象が1〜2件程度ならゲーム名を書いてよい）。「いろんな」のような口語表現は使わず、「様々な」「いくつかの」など落ち着いた言い方にする。

`date`は執筆日ではなく、実際に`main`へマージ（本番反映）された日にする。同日に複数エントリがある場合は、配列内の並び順を実際にマージされた時刻の新しい順（配列の上ほど新しい）に揃える。エントリを追記・修正する際は`git log main`でマージ時刻を確認してから並べる。
