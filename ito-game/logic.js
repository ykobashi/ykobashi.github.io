...
// バンクから未出題(usedに含まれない)の1件を選ぶ。キーはentry.title。
// 候補が尽きた場合は出題履歴をリセットしてバンク全体から選び直す。
// 返り値の used を次回呼び出しに渡すことで、部屋内での重複出題を防ぐ。
function selectRoundTheme(rng = Math.random, bank = THEME_BANK, used = []) {
  const usedSet = new Set(used);
  const pool = bank.filter((entry) => !usedSet.has(entry.title));

  if (pool.length > 0) {
    const entry = pool[Math.min(pool.length - 1, Math.floor(rng() * pool.length))];
    return { entry, used: used.concat([entry.title]) };
  }

  const entry = bank[Math.min(bank.length - 1, Math.floor(rng() * bank.length))];
  return { entry, used: [entry.title] };
}
