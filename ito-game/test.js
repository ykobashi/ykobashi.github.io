...
// selectRoundTheme: usedにない語だけが候補になる
{
  const bank = THEME_BANK.slice(0, 4);
  const used = [bank[0].title, bank[1].title];
  for (const r of [0, 0.4, 0.9]) {
    const result = selectRoundTheme(() => r, bank, used);
    assert.ok([bank[2].title, bank[3].title].includes(result.entry.title));
    assert.deepStrictEqual(result.used, used.concat([result.entry.title]));
  }
}

// selectRoundTheme: プールが尽きたら履歴がリセットされて1件になる
{
  const bank = THEME_BANK.slice(0, 2);
  const used = [bank[0].title, bank[1].title];
  const result = selectRoundTheme(() => 0.5, bank, used);
  assert.ok(bank.includes(result.entry));
  assert.strictEqual(result.used.length, 1);
}

// selectRoundTheme: 返り値のusedの長さは呼ぶたびに1ずつ増える
{
  let used = [];
  for (let i = 0; i < THEME_BANK.length; i++) {
    const result = selectRoundTheme(Math.random, THEME_BANK, used);
