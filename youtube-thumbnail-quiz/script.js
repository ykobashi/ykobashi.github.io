// script.js - YouTubeサムネ当てクイズ UIロジック(DOM操作・ロビー・オンライン対戦の配線)
(function () {
  'use strict';

  const HOST_ID = 'host';
  const L = YoutubeThumbnailQuizLogic;
  const VIDEO_BANK = L.DEFAULT_BANK;
  const GAME_KEY = 'youtube-thumbnail-quiz';
  const REJOIN_GRACE_MS = 30000;
  const MAX_THUMB_RETRY = 4;
  ...
  const resultThumbImg = document.getElementById('result-thumb-img');
  ...
  let answers = {}; // ホストのみが保持する今回の回答マップ {playerId: title}
  let tallied = false;
  ...
  hostBtn.addEventListener('click', () => {
    ...
    net = YoutubeThumbnailQuizNet.hostRoom({
      onCode(code) {
        roomCode = code;
  ...
  joinBtn.addEventListener('click', () => {
    ...
    net = YoutubeThumbnailQuizNet.joinRoom(roomCode, {
      onOwnId(id) {
        myId = id;
      },
      onConnected(c) {
  ...
  function handleHostMessage(peerId, data) {
    ...
    if (data.type === 'answer' && phase === 'question' && data.scopeId === String(currentRound) && data.actionId) {
      ...
      if (!processedActions.has(key)) {
        ...
        acceptAnswer(peerId, data.title);
      }
  ...
  // サムネが読み込めない(動画削除・非公開等)場合、そのエントリを使用済み扱いにして
  // 最大 MAX_THUMB_RETRY 回まで再抽選する。ゲスト側にはこのリトライ処理を一切持たせない。
  //
  // 注意: img.youtube.com は存在しない/削除された動画IDに対してもHTTP 200で
  // 120x90のグレーのプレースホルダー画像を返す(onerrorは発火しない)。そのため
  // onload後にサイズをチェックし、プレースホルダーサイズ以下なら無効な動画とみなす。
  const THUMB_PLACEHOLDER_MAX_SIZE = 120;

  function pickAndBroadcastRound(attempt) {
    ...
    const retryOrFinish = () => {
      if (attempt >= MAX_THUMB_RETRY) {
        finish();
        return;
      }
      pickAndBroadcastRound(attempt + 1);
    };

    const img = new Image();
    img.onload = () => {
      if (img.naturalWidth <= THUMB_PLACEHOLDER_MAX_SIZE && img.naturalHeight <= THUMB_PLACEHOLDER_MAX_SIZE) {
        retryOrFinish();
        return;
      }
      finish();
    };
    img.onerror = retryOrFinish;
    img.src = L.thumbnailUrl(entry.videoId);
  }
  ...
  function enterQuestion(data) {
    ...
    thumbImg.src = L.thumbnailUrl(data.videoId);
  ...
  function submitAnswer(title, btnEl) {
    ...
    answerStatusEl.classList.remove('hidden');
    if (isHost) {
      acceptAnswer(myId, title);
    } else if (conn) {
      const actionId = RejoinStorage.newToken();
      const payload = { type: 'answer', title, actionId, scopeId: String(currentRound) };
  ...
  function acceptAnswer(playerId, title) {
  ...
  function showRoundResult(data) {
    ...
    resultThumbImg.src = L.thumbnailUrl(data.videoId);
