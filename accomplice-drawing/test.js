...
// undoLastStroke: 直近のストローク(末尾の同一strokeIdのかたまり)だけを取り除く。非破壊
{
  const segments = [
    { strokeId: 1, x0: 0 }, { strokeId: 1, x0: 1 },
    { strokeId: 2, x0: 2 }, { strokeId: 2, x0: 3 }, { strokeId: 2, x0: 4 },
  ];
  const result = L.undoLastStroke(segments);
  assert.deepStrictEqual(result, [{ strokeId: 1, x0: 0 }, { strokeId: 1, x0: 1 }]);
  assert.strictEqual(segments.length, 5);
}
assert.deepStrictEqual(L.undoLastStroke([{ strokeId: 1, x0: 0 }, { strokeId: 1, x0: 1 }]), []);
assert.deepStrictEqual(L.undoLastStroke([]), []);
...
// checkCaught: 非共犯側は一人でも本当のペアを言い当てれば「特定成功」（全員一致・多数決は不要）
assert.strictEqual(L.checkCaught(L.tallyPairVotes({ a: ['a', 'b'], b: ['a', 'b'], c: ['a', 'c'] }), ab), true); // 全員正解
assert.strictEqual(L.checkCaught(L.tallyPairVotes({ a: ['a', 'b'], b: ['a', 'c'] }), ab), true); // 同率でも1人正解していれば特定成功
assert.strictEqual(L.checkCaught(L.tallyPairVotes({ a: ['a', 'c'], b: ['a', 'd'] }), ab), false); // 誰も正解していなければ特定失敗
assert.strictEqual(L.checkCaught(L.tallyPairVotes({ a: ['a', 'c'] }), ab), false);
assert.strictEqual(L.checkCaught(L.tallyPairVotes({}), ab), false);
// 共犯自身の投票は特定判定の母数から除外される（自己投票で誤って特定成功にならない）
assert.strictEqual(L.checkCaught(L.tallyPairVotes({ a: ['a', 'b'], b: ['a', 'b'], host: ['a', 'c'] }, ['a', 'b']), ab), false);
assert.strictEqual(L.checkCaught(L.tallyPairVotes({ a: ['a', 'b'], b: ['a', 'b'], host: ['a', 'b'] }, ['a', 'b']), ab), true);
