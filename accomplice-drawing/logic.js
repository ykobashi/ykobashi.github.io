...
// segments(strokeIdを持つ線分の配列)から、直近の1ストローク(同じstrokeIdを持つ末尾のかたまり)を
// 取り除いた新しい配列を返す。非破壊(元配列は変更しない)。空配列ならそのまま空配列を返す
function undoLastStroke(segments) {
  if (!Array.isArray(segments) || segments.length === 0) return Array.isArray(segments) ? segments.slice() : [];
  const lastStrokeId = segments[segments.length - 1].strokeId;
  return segments.filter((segment) => segment.strokeId !== lastStrokeId);
}
...
function checkCaught(tally, truePairKey) {
  ...
  return !!tally && !!tally.counts && (tally.counts[truePairKey] || 0) > 0;
}
...
const AccompliceDrawingLogicExports = { MIN_PLAYERS, ROUNDS, TOPIC_BANK, shuffle, assignAccomplicePair, distributeTopics, pairKey, parsePairKey, undoLastStroke, tallyPairVotes, checkAllyFound, checkCaught, determineWinner, addPlayer, removePlayer, hasMinPlayers };
