(function () {
  ...
  let roster = [], phase = 'lobby', gameId = 0, round = 0, topic = '', allSegments = [], currentRoundSegments = [], latestDrawings = {}, submitted = false, selectedCandidateIds = [], voted = false;
  ...
  function applyRoundStart(data) {
    ...
    phase = 'drawing'; round = data.round; currentRoundSegments = []; submitted = false; drawing = null; $('round-label').textContent = `第${round}ターン / ${L.ROUNDS}`; $('submit-status').textContent = '他の人には、全員が提出するまで見えません。'; $('reset-turn').disabled = false; $('turn-done').disabled = false; canvas.style.cursor = 'crosshair'; showSection('drawing'); redraw();
  }
  ...
  function submitMine() {
    if (phase !== 'drawing' || submitted) return; submitted = true; $('reset-turn').disabled = true; $('turn-done').disabled = true; canvas.style.cursor = 'not-allowed'; $('submit-status').textContent = '提出済みです。全員の提出を待っています。';
  ...
  function finishVoting() { if(!isHost||phase!=='voting'||hostState.votes.size!==roster.length) return; const tally=L.tallyPairVotes(hostState.votes); const citizenTally=L.tallyPairVotes(hostState.votes,hostState.accomplicePair); const trueKey=L.pairKey(hostState.accomplicePair[0],hostState.accomplicePair[1]); const allyFound=L.checkAllyFound(hostState.accomplicePair,hostState.votes); const caught=L.checkCaught(citizenTally,trueKey); const data={type:'result',gameId,accomplicePair:hostState.accomplicePair.slice(),tally,allyFound,caught,winner:L.determineWinner({allyFound,caught})}; net.broadcast(data); applyResult(data); }
  function applyResult(data) { if(!currentMessage(data,'voting',false)||!Array.isArray(data.accomplicePair)||!data.tally) return; phase='result'; $('result-title').textContent=data.winner==='accomplice'?'🕵️ 共犯チームの勝ち！':'🎉 非共犯チームの勝ち！'; $('true-pair').textContent=`${playerName(data.accomplicePair[0])}さん ＆ ${playerName(data.accomplicePair[1])}さん`; $('result-detail').textContent=`相方発見：${data.allyFound?'成功':'失敗'} ／ 非共犯側による特定：${data.caught?'成功':'失敗'}`; $('counts').textContent=''; Object.keys(data.tally.counts||{}).forEach((key)=>{ let pair; try{pair=L.parsePairKey(key);}catch(_){return;} const li=document.createElement('li'); li.textContent=`${playerName(pair[0])}さんと${playerName(pair[1])}さん：${data.tally.counts[key]}票`; $('counts').appendChild(li); }); $('again').classList.toggle('hidden',!isHost); showSection('result'); }
  function abortGame(message) { if(phase==='lobby'||phase==='result'||phase==='aborted') return; phase='aborted'; $('disconnect').textContent=message||'参加者が切断しました。モード選択に戻ってください。'; $('reset-turn').disabled=true; $('turn-done').disabled=true; showSection('aborted'); }

  function resetCurrentTurn() { if (phase !== 'drawing' || submitted) return; currentRoundSegments = []; drawing = null; redraw(); }
  ...
  $('copy').addEventListener('click',()=>{if(navigator.clipboard)navigator.clipboard.writeText($('room-code').textContent);}); $('start').addEventListener('click',startGame); $('reset-turn').addEventListener('click',resetCurrentTurn); $('turn-done').addEventListener('click',submitMine); $('next-round').addEventListener('click',hostAdvance); $('send-vote').addEventListener('click',sendVote); $('again').addEventListener('click',startGame); $('quit').addEventListener('click',()=>location.reload());
