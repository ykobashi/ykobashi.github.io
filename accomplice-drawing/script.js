(function () {
  ...
  function finishVoting() { if(!isHost||phase!=='voting'||hostState.votes.size!==roster.length) return; const tally=L.tallyPairVotes(hostState.votes); const citizenTally=L.tallyPairVotes(hostState.votes,hostState.accomplicePair); const trueKey=L.pairKey(hostState.accomplicePair[0],hostState.accomplicePair[1]); const allyFound=L.checkAllyFound(hostState.accomplicePair,hostState.votes); const caught=L.checkCaught(citizenTally,trueKey); const drawings={}; roster.forEach((p)=>{drawings[p.id]=hostState.drawings.get(p.id)||[];}); const data={type:'result',gameId,accomplicePair:hostState.accomplicePair.slice(),tally,allyFound,caught,winner:L.determineWinner({allyFound,caught}),topics:Object.assign({},hostState.topics),drawings}; lastResult=data; net.broadcast(data); showResult(data); }
  function renderResultGallery(data) { $('result-gallery').textContent=''; if(!data.topics||!data.drawings) return; roster.forEach((p) => { const segments=data.drawings[p.id]; const topicText=data.topics[p.id]; if(!validSegments(segments,0,15000)||typeof topicText!=='string') return; const card=document.createElement('div'); card.className='drawing-card'; const title=document.createElement('h3'); title.textContent=p.name+(p.id===myId?'（あなた）':''); const topicLabel=document.createElement('p'); topicLabel.className='card-topic'; topicLabel.textContent=topicText; const mini=document.createElement('canvas'); mini.width=320; mini.height=320; const miniCtx=mini.getContext('2d'); miniCtx.strokeStyle='#222'; miniCtx.lineWidth=5; miniCtx.lineCap='round'; segments.forEach((s) => drawSegment(miniCtx, s)); card.append(title, topicLabel, mini); $('result-gallery').appendChild(card); }); }
  function showResult(data) { if(!currentMessage(data,'voting',false)||!Array.isArray(data.accomplicePair)||!data.tally) return; phase='result';lastResult=data;RejoinStorage.clear(GAME_KEY); $('result-title').textContent=data.winner==='accomplice'?'🕵️ 共犯チームの勝ち！':'🎉 非共犯チームの勝ち！'; $('true-pair').textContent=`${playerName(data.accomplicePair[0])}さん ＆ ${playerName(data.accomplicePair[1])}さん`; $('result-ally').textContent=`相方発見：${data.allyFound?'成功':'失敗'}`; $('result-ally').classList.toggle('success',data.allyFound); $('result-ally').classList.toggle('fail',!data.allyFound); $('result-caught').textContent=`非共犯側による特定：${data.caught?'成功':'失敗'}`; $('result-caught').classList.toggle('success',data.caught); $('result-caught').classList.toggle('fail',!data.caught); $('counts').textContent=''; Object.keys(data.tally.counts||{}).forEach((key)=>{ let pair; try{pair=L.parsePairKey(key);}catch(_){return;} const li=document.createElement('li'); li.textContent=`${playerName(pair[0])}さんと${playerName(pair[1])}さん：${data.tally.counts[key]}票`; $('counts').appendChild(li); }); renderResultGallery(data); $('again').classList.toggle('hidden',!isHost); showSection('result'); }
