...
// selectRoundWordEntry: usedにない語だけが候補になる
{
  const bank = WORD_BANK.slice(0, 4);
  const used = [bank[0].word, bank[1].word];
  for (const r of [0, 0.4, 0.9]) {
    const result = selectRoundWordEntry(() => r, bank, used);
    assert.ok([bank[2].word, bank[3].word].includes(result.entry.word));
    assert.deepStrictEqual(result.used, used.concat([result.entry.word]));
  }
}

// selectRoundWordEntry: プールが尽きたら履歴がリセットされて1件になる
{
  const bank = WORD_BANK.slice(0, 2);
  const used = [bank[0].word, bank[1].word];
  const result = selectRoundWordEntry(() => 0.5, bank, used);
  assert.ok(bank.includes(result.entry));
  assert.strictEqual(result.used.length, 1);
}

// selectRoundWordEntry: 返り値のusedの長さは呼ぶたびに1ずつ増える
{
  let used = [];
  for (let i = 0; i < WORD_BANK.length; i++) {
    const result = selectRoundWordEntry(Math.random, WORD_BANK, used);
