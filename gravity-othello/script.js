// script.js - 重力オセロ UIロジック(DOM操作・入力ハンドリング・オンライン対戦の配線)
(function () {
  'use strict';
  ...
  function onCellClick(e) {
    ...
    if (mode === 'online') {
      ...
      if (dropAndCheck(col, myColor)) {
        sendToPeer({ type: 'move', col });
        return;
      }
      sendToPeer({ type: 'move', col });
    ...
    if (dropAndCheck(col, BLACK)) return;
    currentPlayer = WHITE;
    updateTurnIndicator();
    scheduleAiMove();
  }
  ...
  function scheduleAiMove() {
    ...
    setTimeout(() => {
      const move = GravityOthelloLogic.chooseAiMove(board, WHITE, BLACK);
      aiThinking = false;
      boardEl.classList.remove('thinking');
      if (!move || gameOver) return;

      if (dropAndCheck(move.col, WHITE)) return;
  ...
  hostBtn.addEventListener('click', () => {
    ...
    onlinePeer = GravityOthelloNet.hostRoom({
      onCode(code) {
  ...
  joinBtn.addEventListener('click', () => {
    ...
    onlinePeer = GravityOthelloNet.joinRoom(code, {
      onConnected(conn) {
        onlineConn = conn;
        myColor = WHITE;
        enterOnlineGame();
  ...
  function handlePeerMessage(data) {
    ...
    if (data.type === 'move') {
      if (gameOver) return;
      const opponentColor = GravityOthelloLogic.otherPlayer(myColor);
      if (!GravityOthelloLogic.canDropInColumn(board, data.col)) return;
      if (dropAndCheck(data.col, opponentColor)) return;
