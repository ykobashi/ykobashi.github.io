...
// 再戦のたびに buildTargetChain / assignWords が(規定rngを使わず既定のMath.randomで)独立にランダム化されることを検証する。
// 同じプレイヤー集合で何度も呼び出し、十分な試行回数で複数パターンが出ることを統計的に確認する(順番固定・シード固定の回帰防止)。
const rematchIds=['host','g1','g2','g3'];
const chainPatterns=new Set(), targetPatterns=new Set();
for (let i=0;i<40;i++){ const {order,targetOf}=L.buildTargetChain(rematchIds); chainPatterns.add(order.join(',')); targetPatterns.add(rematchIds.map(id=>targetOf[id]).join(',')); }
assert(chainPatterns.size>1, 'buildTargetChainが再戦のたびに同じ並びしか生成していません');
assert(targetPatterns.size>1, 'buildTargetChainの循環関係(誰が誰を当てるか)が再戦のたびに固定されています');
const wordPatterns=new Set();
for (let i=0;i<40;i++){ const words=L.assignWords(rematchIds); wordPatterns.add(rematchIds.map(id=>words[id]).join(',')); }
assert(wordPatterns.size>1, 'assignWordsが再戦のたびに同じ割り当てしか生成していません');
