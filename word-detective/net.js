...
(function () {
  ...
  const ROOM_PREFIX = 'worddetective-ykobashi-';
  ...
  function hostRoom(handlers, attemptsLeft) {
    ...
    return {
      ...
      broadcast(data) {
        conns.forEach((c) => {
          if (c.open) {
            try { c.send(data); } catch (err) { handlers.onError(err, c.peer); }
          }
        });
      },
      sendTo(peerId, data) {
        ...
        if (c && c.open) {
          try { c.send(data); } catch (err) { handlers.onError(err, peerId); }
        }
      },
      peerIds() {
