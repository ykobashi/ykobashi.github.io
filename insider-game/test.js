...
// selectRoundTopic: usedにない語だけが候補になる
{
  const bank = ['a', 'b', 'c', 'd'];
  const used = ['a', 'b'];
  for (const r of [0, 0.3, 0.6, 0.99]) {
    const result = selectRoundTopic(() => r, bank, used);
    assert.ok(['c', 'd'].includes(result.topic));
    assert.deepStrictEqual(result.used, used.concat([result.topic]));
  }
}

// selectRoundTopic: プールが尽きたら履歴がリセットされて1件になる
{
  const bank = ['a', 'b'];
  const used = ['a', 'b'];
  const result = selectRoundTopic(() => 0.5, bank, used);
  assert.ok(bank.includes(result.topic));
  assert.strictEqual(result.used.length, 1);
}

// selectRoundTopic: 返り値のusedの長さは呼ぶたびに1ずつ増える(バンクを使い切るまで)
{
  let used = [];
  for (let i = 0; i < TOPIC_BANK.length; i++) {
    const result = selectRoundTopic(Math.random, TOPIC_BANK, used);
    assert.strictEqual(result.used.length, i + 1);
    used = result.used;
  }
  // バンクを使い切った次の呼び出しでは履歴がリセットされる
  const afterFull = selectRoundTopic(Math.random, TOPIC_BANK, used);
  assert.strictEqual(afterFull.used.length, 1);
}
