...
// undoLastStroke: 直近のストローク(末尾の同一strokeIdのかたまり)だけを取り除く。非破壊
{
  ...
  const result = undoLastStroke(segments);
  assert.deepStrictEqual(result, [{ strokeId: 1, x0: 0 }, { strokeId: 1, x0: 1 }]);
  assert.strictEqual(segments.length, 5, 'undoLastStroke should not mutate the original array');
}
{
  // 単一ストロークのみの場合は空配列になる
  const segments = [{ strokeId: 1, x0: 0 }, { strokeId: 1, x0: 1 }];
  assert.deepStrictEqual(undoLastStroke(segments), []);
}
{
  // 空配列はそのまま空配列を返す
  assert.deepStrictEqual(undoLastStroke([]), []);
}
