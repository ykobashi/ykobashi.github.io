// script.js - 偽物鑑定ゲーム UIロジック(DOM操作・ロビー・オンライン対戦の配線)
(function () {
  'use strict';

  const HOST_ID = 'host';
  const L = RealOrFakePhotoLogic;
  const GAME_KEY = 'real-or-fake-photo';
  const REJOIN_GRACE_MS = 30000;
  const ALLOWED_IMAGE_PATHS = L.allImagePaths(L.PAIR_BANK);
  ...
  const resultPhotoImgs = [document.getElementById('result-photo-img-0'), document.getElementById('result-photo-img-1')];
  const resultPhotoLabels = [document.getElementById('result-photo-label-0'), document.getElementById('result-photo-label-1')];
  const resultCreditTextEl = document.getElementById('result-credit-text');
  ...
  let usedPairIds = []; // ホストのみが保持する、この部屋で出題済みのペア(重複出題防止)
  let currentRound = 0;
  let pendingAnswer = null;
  const processedActions = new Set();
  let currentPair = null; // ホストのみが保持する今回のペア {id, images:[a,b], correctIndex, credit}
  let answers = {}; // ホストのみが保持する今回の回答マップ {playerId: selectedIndex}
  let tallied = false;
  ...
  hostBtn.addEventListener('click', () => {
    ...
    net = RealOrFakePhotoNet.hostRoom({
      onCode(code) {
        roomCode = code;
  ...
  joinBtn.addEventListener('click', () => {
    ...
    net = RealOrFakePhotoNet.joinRoom(roomCode, {
      onOwnId(id) {
        myId = id;
      },
      onConnected(c) {
  ...
  function handleHostMessage(peerId, data) {
    ...
    if (data.type === 'answer' && phase === 'question' && data.scopeId === String(currentRound) && data.actionId) {
      ...
      if (!processedActions.has(key)) {
        ...
        acceptAnswer(peerId, data.selectedIndex);
      }
    ...
    if (data.type === 'state-snapshot' && data.snapshotVersion === 1) {
      ...
      if (data.phase === 'question' && data.question) {
        ...
        if (data.myAnswer !== null && data.myAnswer !== undefined) {
          ...
          highlightSelected(data.myAnswer);
  ...
  function renderPhotoChoices(images) {
    images.forEach((path, i) => {
      setImage(photoImgs[i], path);
      photoBtns[i].classList.remove('selected');
      photoBtns[i].disabled = false;
    });
  }

  function setPhotoButtonsDisabled(disabled) {
    photoBtns.forEach((btn) => { btn.disabled = disabled; });
  }

  function highlightSelected(index) {
    photoBtns.forEach((btn, i) => { btn.classList.toggle('selected', i === index); });
  }
  ...
  function submitAnswer(selectedIndex) {
    ...
    answerStatusEl.classList.remove('hidden');
    if (isHost) {
      acceptAnswer(myId, selectedIndex);
    } else if (conn) {
      const actionId = RejoinStorage.newToken();
      const payload = { type: 'answer', selectedIndex, actionId, scopeId: String(currentRound) };
  ...
  function acceptAnswer(playerId, selectedIndex) {
  ...
  function tallyRound() {
    if (!isHost || tallied || !currentPair) return;
    tallied = true;
    const tally = L.tallyRoundAnswers(answers, currentPair.correctIndex);
    ...
    const payload = {
      ...
      totalRounds: L.ROUND_TOTAL,
      images: currentPair.images,
      correctIndex: currentPair.correctIndex,
      credit: currentPair.credit,
  ...
  function showRoundResult(data) {
    ...
    data.images.forEach((path, i) => {
      setImage(resultPhotoImgs[i], path);
      const isReal = i === data.correctIndex;
      resultPhotoLabels[i].textContent = isReal ? '本物' : '偽物(AI生成)';
      resultPhotoImgs[i].closest('.photo-result-slot').classList.toggle('is-real', isReal);
    });
    resultCreditTextEl.innerHTML = '';
    if (data.credit) {
      const link = document.createElement('a');
      link.href = data.credit.source;
      link.target = '_blank';
      link.rel = 'noopener noreferrer';
      link.textContent = data.credit.title + '（' + (data.credit.creator || '作者不明') + '）';
      resultCreditTextEl.appendChild(document.createTextNode('本物の出典: '));
      resultCreditTextEl.appendChild(link);
    }
