// script.js - 辞書定義当てクイズ UIロジック(DOM操作・ロビー・オンライン対戦の配線)
(function () {
  'use strict';

  const HOST_ID = 'host';
  const L = DictionaryQuizLogic;
  const GAME_KEY = 'dictionary-quiz';
  ...
  let currentEntry = null; // ホストのみが保持する今回の正解語 {word, reading, meaning}
  let answers = {}; // ホストのみが保持する今回の回答マップ {playerId: word}
  let tallied = false;
  ...
  hostBtn.addEventListener('click', () => {
    ...
    net = DictionaryQuizNet.hostRoom({
      onCode(code) {
        roomCode = code;
  ...
  joinBtn.addEventListener('click', () => {
    ...
    net = DictionaryQuizNet.joinRoom(roomCode, {
      onOwnId(id) {
        myId = id;
      },
      onConnected(c) {
  ...
  function enterQuestion(data) {
    ...
    meaningTextEl.textContent = data.meaning;
  ...
  function renderChoices(choices) {
    ...
    choices.forEach((word) => {
      ...
      btn.addEventListener('click', () => submitAnswer(word, btn));
      choicesListEl.appendChild(btn);
    });
  }

  function submitAnswer(word, btnEl) {
    ...
    answerStatusEl.classList.remove('hidden');
    if (isHost) {
      acceptAnswer(myId, word);
    } else if (conn) {
      const actionId = RejoinStorage.newToken();
      const payload = { type: 'answer', word, actionId, scopeId: String(currentRound) };
  ...
  function acceptAnswer(playerId, word) {
  ...
  function tallyRound() {
    ...
    tallied = true;
    const tally = L.tallyRoundAnswers(answers, currentEntry.word);
  ...
  function showRoundResult(data) {
    ...
    resultReadingEl.textContent = '（' + data.reading + '）';
    resultMeaningEl.textContent = data.meaning;
