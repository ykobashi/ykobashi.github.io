(function () {
  ...
  function castVote(target, button) { voted = true; [...$('candidates').children].forEach((b) => b.classList.remove('selected')); button.classList.add('selected'); $('vote-status').textContent = '投票しました。（集計開始まで変更できます）'; if (isHost) { votes[myId] = target; updateProgress(); } else conn.send({ type: 'vote', target }); }
